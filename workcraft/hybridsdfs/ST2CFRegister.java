...
public class ST2CFRegister extends SDFSRegisterBase   {
	public static final UUID _modeluuid = UUID.fromString("aab78c50-e6bf-11dc-95ff-0800200c9a66");
	public static final String _displayname = "ST-CF Register";
	...
	public ST2CFRegister(BasicEditable parent) throws UnsupportedComponentException {
		super(parent);
	...
	public void doDraw(Painter p) {
		...
		if (state == RegisterState.ENABLED)
			...
			p.setFillColor(disabled_color);
		p.drawRect(-0.08f, 0.05f, 0.0f, -0.05f);
		...
		p.drawRect(0.0f, 0.05f, 0.08f, 0.0f);

		if (backward_state == RegisterState.ENABLED)
			p.setFillColor(enabled_color);
		else
			p.setFillColor(disabled_color);
		p.drawRect(0.0f, 0.0f, 0.08f, -0.05f);
		
		p.drawLine(0.0f, 0.0f, 0.08f, 0.0f);
	...
	public void fromXmlDom(Element element) throws DuplicateIdException {
		NodeList nl = element.getElementsByTagName("st2cf-register");
	...
	public Element toXmlDom(Element parent_element) {
		...
		Element ppe = d.createElement("st2cf-register");
	...
	public void rebuildRuleFunctions() {
		...
		markFunc = expandRule("self re,r-preset m,self !om,self !am");
		unmarkFunc = expandRule("self !re,r-preset !m,self om,self am");
		...
		fwdEnableFunc = expandRule("self !am,self m");
		fwdDisableFunc = expandRule("self am,self !m");
